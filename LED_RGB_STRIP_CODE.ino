...
byte activeFrequencyBand = 1;
RgbColor filteredAudioColor = {0, 0, 0};
bool audioColorReady = false;
...
const int BASS_MAX_HZ = 250;
const int MID_MAX_HZ = 1200;
const int MAX_USABLE_FREQUENCY_HZ = 3500;
const byte FREQUENCY_COLOR_SMOOTH_AMOUNT = 42;
...
// Each row is: bass, mid, treble. Brightness is controlled separately by volume.
const RgbColor MODE_BAND_COLORS[5][3] = {
  // GAME: amber, gold, bright yellow (clearly different from WAVE)
  {{145, 55, 0}, {255, 185, 0}, {255, 250, 85}},

  // WAVE: bass navy, mid ocean blue, treble ice blue
  {{0, 20, 150}, {0, 135, 255}, {120, 255, 255}},

  // RAIN: bass purple, mid indigo, treble sky blue
  {{115, 0, 225}, {50, 40, 255}, {0, 210, 255}},

  // CHILL: teal, mint, icy cyan (avoids unstable lavender mixing on analog strips)
  {{0, 105, 105}, {0, 210, 150}, {45, 255, 225}},

  // FIRE: bass deep red, mid orange, treble warm yellow
  {{205, 0, 0}, {255, 75, 0}, {255, 235, 15}}
};
...
void resetAudioBeat() {
  ...
  activeFrequencyBand = 1;
  audioColorReady = false;
  filteredAudioColor.red = 0;
  filteredAudioColor.green = 0;
  filteredAudioColor.blue = 0;
}
...
byte frequencyBandIndex(int frequencyHz) {
  // Keep the last valid band if the short sampling window cannot estimate a frequency.
  if (frequencyHz <= 0 || frequencyHz > MAX_USABLE_FREQUENCY_HZ) {
    return activeFrequencyBand;
  }

  if (frequencyHz < BASS_MAX_HZ) return 0;
  if (frequencyHz < MID_MAX_HZ) return 1;
  return 2;
}

RgbColor colorForModeFrequency(int frequencyHz) {
  activeFrequencyBand = frequencyBandIndex(frequencyHz);
  return MODE_BAND_COLORS[modeIndex()][activeFrequencyBand];
}

void showVolumeOnRgbStrip(const AudioData &audio) {
  ...
  int volumePercent = constrain(audio.volumePercent, 0, 100);
  ...
  RgbColor targetColor = colorForModeFrequency(audio.frequencyHz);
  if (!audioColorReady) {
    filteredAudioColor = targetColor;
    audioColorReady = true;
  } else {
    filteredAudioColor = blendRgbColor(
      filteredAudioColor,
      targetColor,
      FREQUENCY_COLOR_SMOOTH_AMOUNT
    );
  }

  byte brightness = map(beatLevel, 0, 255, STRIP_MIN_BRIGHTNESS, 255);

  currentVolumeLevel = map(beatLevel, 0, 255, 1, 7);

  setRgbColor(
    (byte)((unsigned int)filteredAudioColor.red * brightness / 255),
    (byte)((unsigned int)filteredAudioColor.green * brightness / 255),
    (byte)((unsigned int)filteredAudioColor.blue * brightness / 255)
  );
}
...
const char *frequencyBandName(int frequencyHz) {
  ...
  if (frequencyHz < BASS_MAX_HZ) return "BASS";
  if (frequencyHz < MID_MAX_HZ) return "MID";
  if (frequencyHz <= MAX_USABLE_FREQUENCY_HZ) return "TREBLE";
  return "OUT_OF_RANGE";
}
